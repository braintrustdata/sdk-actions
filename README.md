...
repository pushes its package tags, call `release/create-npm-package-github-releases`; it
...
`publish-npm-tarballs` derives it from `name` and `version`. `sbom_asset` is required
by `create-npm-package-github-releases` and is resolved relative to the manifest
file's directory. `tag` defaults to `name@version`; `release_title` defaults to `tag`; and
